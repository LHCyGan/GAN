...
#
# from model import Generator, Discriminator, discriminator_loss, generator_loss, cycle_consistency_loss
#
# os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'
#
# np.random.seed(998)
# tf.random.set_seed(996)
...
# # True尝试将文件提取为存档文件，例如tar或zip。
# # 保存文件的Keras缓存目录下的子目录。 如果指定了绝对路径/ path / to / folder，则文件将保存在该位置。
# #return Path to the downloaded file
# path_to_zip = keras.utils.get_file(fname='horse2zebra.zip',
...
# train_datasetA = train_datasetA.repeat(epochs).shuffle(trainA_size).map(load_image)
...
# train_datasetB = train_datasetB.repeat(epochs).shuffle(trainB_size).map(load_image)
# train_datasetB = train_datasetB.batch(batch_size).prefetch(batch_size)
...
#
#
# def generator_image(A, B, A2B, B2A, epoch):
#
...
#         plt.axis('off')
#     plt.show()
#     plt.savefig("./images/image_%d.png" % epoch)
#     plt.close()
#
#     return None
#
...
#
#             reconstructA = genB2A(genA2B_output, training=True)
#             reconstructB = genA2B(genB2A_output, training=True)
#
...
#
#             genA2B_loss = generator_loss(discB_fake_output, lsgan=lsgan) + cycle_consistency_loss(trainA, trainB, reconstructA, reconstructB, cyc_lambda)
#             genB2A_loss = generator_loss(discA_fake_output, lsgan=lsgan) + cycle_consistency_loss(trainA, trainB, reconstructA, reconstructB, cyc_lambda)
#
#         genA2B_gradient = genA2B_tape.gradient(genA2B_loss, genA2B.trainable_variables)
#         genB2A_gradient = genB2A_tape.gradient(genB2A_loss, genB2A.trainable_variables)
#
#         discA_gradient = discA_tape.gradient(discA_loss, discA.trainable_variables)
#         discB_gradient = discB_tape.gradient(discB_loss, discB.trainable_variables)
#
#         genA2B_optimizer.apply_gradients(zip(genA2B_gradient, genA2B.trainable_variables))
#         genB2A_optimizer.apply_gradients(zip(genB2A_gradient, genB2A.trainable_variables))
#
#         discA_optimizer.apply_gradients(zip(discA_gradient, discA.trainable_variables))
#         discB_optimizer.apply_gradients(zip(discB_gradient, discB.trainable_variables))
#
#         if epoch % 40 == 0:
#             generator_image(trainA, trainB, genA2B_output, genB2A_output, epoch)
#
#             print('Time taken for epoch {} is {} sec'.format(epoch + 1, time.time() - start))
#
#
# if __name__ == '__main__':
#     train(train_datasetA, train_datasetB, epochs, lsgan=True, cyc_lambda=20)
#
#
#
#
...
train_datasetA = train_datasetA.map(load_image)
train_datasetA = train_datasetA.batch(batch_size)
train_datasetA = train_datasetA.prefetch(batch_size)
train_datasetA = iter(train_datasetA)
...
train_datasetB = train_datasetB.map(load_image)
