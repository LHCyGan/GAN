...
def save_result(val_out, val_block_size, img_path, color_mode):
    def preprocess(img):
...
def g_loss_fn(generator, discrimator, batch_z, training):
    ...
    g_loss_ = celoss_ones(d_fake_logits)

    return g_loss_

def gradient_penalty(discrimator, batch_x, fake_image):
    ...
    interplate = batch_x * t + (1 - t) * fake_image

    with tf.GradientTape() as tape:
        tape.watch([interplate])
        logits = discrimator(interplate)
    grads = tape.gradient(logits, interplate)

    grads = tf.reshape(grads, [grads.shape[0], -1])
    g_p = tf.norm(grads, axis=1)
    g_p = tf.reduce_mean((g_p - 1) ** 2)
    return g_p
...
def d_loss_fn(generator, discrimator, batch_x, batch_z, training):
    ...
    g_p = gradient_penalty(discrimator, batch_x, fake_image)

    loss = d_loss_fake + d_loss_real + 1.0 * g_p
    return loss
